...
function compile_defs(defs, main, opts) {
  ...
  if (isio) {
    ...
    code += "        }\n";
    code += "        if (rec.buf.length) { var s = rec.buf.toString('utf8'); rec.buf = Buffer.alloc(0); res('0\\n' + s); return true; }\n";
    code += "        return false;\n";
    code += "      };\n";
    code += "      if (take()) return;\n";
    code += "      rec.wait = () => { take() || res('0\\n'); };\n";
    code += "      setTimeout(() => { if (rec.wait) { rec.wait = null; res('0\\n'); } }, 3000);\n";
    code += "    });\n";
    code += "  };\n";
    }
    if (hneed.ws) {
